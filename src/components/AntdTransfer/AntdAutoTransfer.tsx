...
export const AntdAutoTransfer: React.FC<AntdAutoTransferProps> = (props) => {
    ...
    if (limitMaxCount) {
      ...
      if (sourceSelectedKeys.length === 1) {
        const canSelectCount =
          limitMaxCount - tKeys.length - sourceSelectedKeys.length;
        // 此时左侧的数据
      ...
      if (targetSelectedKeys.length === 1) {
        ...
        const canSelectCount =
          limitMaxCount - tKeys.length + targetSelectedKeys.length;
        // 此时左侧的数据：先在tKeys过滤掉此时点击的选项，然后取右侧数据并取反，就是左侧数据
        const sourceSelectedList = dataSource.filter((item: ItemProps) => {
          return !tKeys
            .filter((i: string) => !tKeys.includes(i))
            .includes(item[idKey]);
        });

        const newTKeys = tKeys.filter(
          (i: string) => !targetSelectedKeys.includes(i)
        );
  ...
  const isDisabled = (sourList: any[], canCount: number, targetKeys: any[]) => {
    ...
    const targetList = dataSource.filter((item) =>
      targetKeys.includes(item[idKey])
    );
